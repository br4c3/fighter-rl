...
class CompetitionBatchDogfight:
    ...
    def reset(self, mask=None):
        ...
        if self.domain_randomization:
            ...
            target_alt = (alt + torch.empty_like(alt).uniform_(-1500, 1500)).clamp(1000, 12000)
            target_speed = torch.empty_like(speed).uniform_(180, 320)
            target_roll = torch.empty_like(roll).uniform_(-45, 45)
            target_pitch = torch.empty_like(pitch).uniform_(-12, 12)
            target_heading = heading + torch.empty_like(heading).uniform_(130, 230)
    ...
    def step(self, action):
        ...
        _, _, target_ata, _, _, _ = self._geometry(t, o)
        # fmt: off
        terminated = (self.own_health <= 0) | (self.target_health <= 0) | (-o[:,2] < 300) | (-t[:,2] < 300)
        # fmt: on
