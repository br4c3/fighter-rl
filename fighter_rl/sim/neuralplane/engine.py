...
EngineState = namedtuple("EngineState", ("n1", "n2", "thrust_lbf", "fuel_flow_pph"))


class CompetitionF100(torch.nn.Module):
    """XML thrust maps are exact; spool time constants are exposed for fitting."""

    def __init__(self, xml_path, hz=60, spool_up_s=1.0, spool_down_s=1.5):
        super().__init__()
        self.dt = 1.0 / float(hz)
        root = ET.parse(xml_path).getroot()
        self.mil = float(root.findtext("milthrust"))
        self.maximum = float(root.findtext("maxthrust"))
    ...
    def _map(self, name, mach, altitude_ft):
        return _interp2(
            mach,
            altitude_ft,
            getattr(self, name + "_mach"),
            getattr(self, name + "_alt"),
            getattr(self, name + "_map"),
        )
    ...
    def forward(self, state, throttle_cmd, mach, altitude_ft):
        ...
        target_n2 = torch.where(lever > 1, target_n2.new_tensor(100.0), target_n2)
