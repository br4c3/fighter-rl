...
def geodetic_to_ecef(lat, lon, alt):
    a = lat.new_tensor(A_FT)
    b = lat.new_tensor(B_FT)
    e2 = 1 - (b / a).square()
...
def geocentric_to_ecef(lat_gc, lon, alt):
    a = lat_gc.new_tensor(A_FT)
    b = lat_gc.new_tensor(B_FT)
    sl, cl = torch.sin(lat_gc), torch.cos(lat_gc)
    so, co = torch.sin(lon), torch.cos(lon)
...
def ecef_to_geocentric(r):
    ...
    b = x.new_tensor(B_FT)
    lon = torch.atan2(y, x)
    rxy = torch.sqrt(x.square() + y.square())
    radius = torch.linalg.vector_norm(r, dim=1).clamp_min(1.0)
