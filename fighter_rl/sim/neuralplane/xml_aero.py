            ...
            for function in axis.findall("function"):
                ...
                for child in product:
                    if child.tag == "property":
                        ...
                        independent = [
                            (v.text or "").strip() for v in child.findall("independentVar")
                        ]
                        lines = [
            ...
            for _name, factors in self.specs[axis]:
                ...
                for kind, item in factors:
                    if kind == "property":
                        ...
                        prefix, key = item
                        value = value * _interp1(
                            properties[key],
                            getattr(self, prefix + "_x"),
                            getattr(self, prefix + "_v"),
                        )
                    else:
                        prefix, row, col = item
                        value = value * _interp2(
                            properties[row],
                            properties[col],
                            getattr(self, prefix + "_r"),
                            getattr(self, prefix + "_c"),
                            getattr(self, prefix + "_v"),
                        )
                total = total + value
...
def airborne_properties(state12, surfaces_deg, speedbrake_deg=None, gear_pos=None):
    ...
    return {
        ...
        "aero/beta-rad": beta,
        "aero/bi2vel": 30.0 / (2 * vt.clamp_min(1)),
        "aero/ci2vel": 11.32 / (2 * vt.clamp_min(1)),
        "aero/function/kCLge": torch.ones_like(vt),
        "aero/h_b-mac-ft": torch.full_like(vt, 2.0),
        "aero/qbar-psf": qbar,
        "fcs/aileron-pos-rad": aero_aileron,
        "fcs/elevator-pos-rad": elevator,
        "fcs/flaperon-mix-rad": flaperon_mix,
        "fcs/lef-pos-rad": lef,
        "fcs/rudder-pos-rad": rudder,
        "fcs/speedbrake-pos-rad": sb,
        "gear/gear-pos-norm": gear,
        "metrics/Sw-sqft": torch.full_like(vt, 300.0),
        "metrics/bw-ft": torch.full_like(vt, 30.0),
        "metrics/cbarw-ft": torch.full_like(vt, 11.32),
