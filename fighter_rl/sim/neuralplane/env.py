...
class CompetitionNeuralPlane(torch.nn.Module):
    ...
    def step(self, action):
        ...
        if self.use_eci:
            ...
            new_eci_position = (
                self.eci_position
                + self.dt*(23*self.eci_velocity - 16*self.eci_velocity_history[:,0] + 5*self.eci_velocity_history[:,1])/12
            )
            new_eci_velocity = self.eci_velocity + self.dt*(1.5*accel_i - 0.5*self.eci_accel_history[:,0])
            # fmt: on
