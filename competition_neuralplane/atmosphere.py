...
def standard_atmosphere(altitude_ft: torch.Tensor):
    ...
    p11 = altitude_ft.new_tensor(SL_PRESSURE_PSF) * (
        altitude_ft.new_tensor(SL_TEMPERATURE_R) / t11).pow(exponent)
...
def standard_temperature(altitude_ft: torch.Tensor):
    """ISA temperature in Rankine for the competition altitude envelope."""
    h = altitude_ft * EARTH_RADIUS_FT / (EARTH_RADIUS_FT + altitude_ft)
    lapse = (389.97 - SL_TEMPERATURE_R) / 36089.2388
    return torch.where(h < 36089.2388, SL_TEMPERATURE_R + lapse*h,
                       altitude_ft.new_tensor(389.97))


def standard_pressure(altitude_ft: torch.Tensor):
    temperature=standard_temperature(altitude_ft)
    h=altitude_ft*EARTH_RADIUS_FT/(EARTH_RADIUS_FT+altitude_ft)
    lapse=(389.97-SL_TEMPERATURE_R)/36089.2388
    exponent=G_ISA/(R_DRY*lapse)
    p_trop=SL_PRESSURE_PSF*(SL_TEMPERATURE_R/temperature).pow(exponent)
    h11=altitude_ft.new_tensor(36089.2388);t11=altitude_ft.new_tensor(389.97)
...
def calibrated_airspeed(true_speed_fps: torch.Tensor, altitude_ft: torch.Tensor):
    """Torch port of FGAuxiliary::VcalibratedFromMach, returned in ft/s."""
    _,sound=standard_atmosphere(altitude_ft);mach=true_speed_fps/sound
    pressure=standard_pressure(altitude_ft);gamma=1.4
    a=(gamma-1)/2;b=gamma/(gamma-1);c=2*b;d=1/(gamma-1)
    sub_total=pressure*(1+a*mach.square()).pow(b)
    coeff=(.5*(gamma+1))**b*((gamma+1)/(gamma-1))**d
    sup_total=pressure*coeff*mach.clamp_min(1).pow(c)/(c*mach.clamp_min(1).square()-1).pow(d)
    ...
    iterate_coeff=(.5*(gamma+1))**(-.25*(2/((gamma-1)/gamma)))*(.5*(gamma+1)/gamma)**(-.5*(.5*2/(gamma-1)))
    for _ in range(10):
        sup=iterate_coeff*torch.sqrt(A*(1-1/((2/((gamma-1)/gamma))*ma.clamp_min(1e-3).square())).clamp_min(0).pow(.5*2/(gamma-1)))
        ma=torch.where(ma>1,sup,ma)
    return (GAMMA*R_DRY*SL_TEMPERATURE_R)**.5*ma


def spherical_gravity(altitude_ft: torch.Tensor):
