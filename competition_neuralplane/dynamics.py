"""Batched 6-DoF rigid-body dynamics driven by competition XML forces."""
...
class CompetitionDynamics(torch.nn.Module):
    def __init__(self):
        ...
        dry_mass=self.dry_weight/32.174; fuel_mass=full_fuel/32.174
        ddx=(full_cgx-self.dry_cgx)/12;ddz=(full_cgz-self.dry_cgz)/12
        tx=(full_cgx-self.tank_x)/12;ty=self.tank_y/12;tz=(full_cgz-self.tank_z)/12
        self.dry_Jx=15963.767196323115-dry_mass*ddz**2-fuel_mass*(ty**2+tz**2)
        self.dry_Jy=57380.332084958936-dry_mass*(ddx**2+ddz**2)-fuel_mass*(tx**2+tz**2)
    ...
    def mass_properties(self, fuel_lbs, reference):
        ...
        dm=reference.new_tensor(self.dry_weight/32.174);fm=fuel_lbs/32.174
        ddx=(cgx-self.dry_cgx)/12;ddz=(cgz-self.dry_cgz)/12
        tx=(cgx-self.tank_x)/12;ty=reference.new_tensor(self.tank_y/12);tz=(cgz-self.tank_z)/12
    ...
    def forward(self, state, aero, thrust_lbf, fuel_lbs=None):
        ...
        lat=x.new_tensor(self.latitude_rad);om=x.new_tensor(self.earth_rate)
        omega_n=om*torch.cos(lat);omega_d=-om*torch.sin(lat)
        ...
        safe_vt = vt.clamp_min(1.)
        vt_d = (u*ud+v*vd+w*wd)/safe_vt
        alpha_d = (u*wd-w*ud)/(u.square()+w.square()).clamp_min(1.)
        beta_d = (vd*safe_vt-v*vt_d)/(safe_vt.square()*cb).clamp_min(1.)
        ...
        rxm,rym,rzm=rhs.unbind(1);denom=Jx*Jz-Jxz.square()
        pqri_dot=torch.stack(((Jz*rxm+Jxz*rzm)/denom,
                              rym/Jy,
                              (Jxz*rxm+Jx*rzm)/denom),1)
        ...
        return torch.stack((north,east,altitude,phi_d,theta_d,psi_d,
                            vt_d,alpha_d,beta_d,pd,qd,rd),1)
