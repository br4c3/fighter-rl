...
class FCSState(NamedTuple):
    roll_i: torch.Tensor
    pitch_i: torch.Tensor
    yaw_i: torch.Tensor
    roll_prev: torch.Tensor
    pitch_prev: torch.Tensor
    yaw_prev: torch.Tensor
...
def initial_fcs_state(batch: int, device=None, dtype=torch.float32):
...
def interp1(x, xp, fp):
    ...
    xp = x.new_tensor(xp); fp = x.new_tensor(fp)
...
class CompetitionF16FCSTorch(torch.nn.Module):
    ...
    def forward(self, state: FCSState, action, aircraft_state, angular_accel=None,
                pilot_loads=None, gravity_attitude=None):
        ...
        gravity_phi,gravity_theta=(phi,theta) if gravity_attitude is None else (gravity_attitude[:,0],gravity_attitude[:,1])
        ...
        if pilot_loads is not None:
            ...
            rotational=torch.linalg.cross(angular_accel,lever)+torch.linalg.cross(omega,torch.linalg.cross(omega,lever))
        ...
        roll_integrate = (aircraft_state[:, 12] * 1.94384449 < 20.)
        ri_candidate = state.roll_i + (1.5*re-.5*state.roll_prev) * self.dt
        ri = torch.where(roll_integrate, ri_candidate, state.roll_i)
        rd = (re - state.roll_prev) / self.dt
        roll_pid = 3.0 * re + .00050 * ri - .00125 * rd
        roll_target = (roll_pid + roll).clamp(-1, 1)
        an = rate_limit(state.aileron, roll_target, .3, self.dt)
        mach_gain = interp1(mach, [0., 1.], [1., .15])
        aileron_deg = an * mach_gain * torch.rad2deg(an.new_tensor(.375))

        # Pitch alpha/g/q feedback and actuator.
        limited = pitch.clamp(-1, .44)
        alpha_gain = interp1(alpha, [-.5236, -.5, 0., .5, .5236], [0., .11, 1., .11, 0.])
        scheduled = limited * alpha_gain
        g_corrected = nz - torch.cos(gravity_theta) * torch.cos(gravity_phi)
        pe = scheduled + 6.2 * q - .020 * g_corrected
        self.last_pitch_scheduled=scheduled
        self.last_pitch_rate=6.2*q
        self.last_g_corrected=g_corrected
        pitch_integrate = (aircraft_state[:, 12] * 1.94384449 < 5.)
        pi_candidate = state.pitch_i + (1.5*pe-.5*state.pitch_prev) * self.dt
        pi = torch.where(pitch_integrate, pi_candidate, state.pitch_i)
        pitch_pid = (.3000 * pe + .0250 * pi).clamp(-1, 1)
        ...
        en = rate_limit(state.elevator, pitch_target, .3, self.dt)
        elevator_deg = en * torch.rad2deg(en.new_tensor(.436))

        # Yaw r/ny feedback.
        yaw_gain = interp1(vg_fps, [80., 100., 150.], [0., 15., 100.])
        ye = rudder + yaw_gain * r + .25 * ny
        yaw_integrate = (aircraft_state[:, 12] * 1.94384449 < 10.)
        yi_candidate = state.yaw_i + (1.5*ye-.5*state.yaw_prev) * self.dt
        yi = torch.where(yaw_integrate, yi_candidate, state.yaw_i)
        yd = (ye - state.yaw_prev) / self.dt
        yaw_pid = (.1055 * ye + .000010 * yi + .00005 * yd).clamp(-1, 1)
        ...
        rn = rate_limit(yaw_pid, yaw_target, .4, self.dt)
        rudder_deg = rn * torch.rad2deg(rn.new_tensor(.524))
        ...
        lef = rate_limit(state.lef, lef_rad * 2.293578, 3., self.dt)
        ...
        tef_rad = torch.where(vc_kts < 250., alpha.new_tensor(.349), alpha.new_zeros(()))
        tef_rad = torch.where(mach > .9, alpha.new_tensor(-.0349), tef_rad)
        tef = rate_limit(state.tef, tef_rad * 2.864789, 3., self.dt)
        tef_deg = tef * 20.
        ...
        aero_aileron_deg = roll_target * torch.rad2deg(an.new_tensor(.375))
        ...
        surfaces = torch.stack((elevator_deg, aileron_deg, rudder_deg, lef_command_deg,
                                tef_deg, aero_aileron_deg, reported_elevator_deg), 1)
